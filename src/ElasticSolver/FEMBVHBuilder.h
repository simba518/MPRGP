#ifndef FEM_BVH_BUILDER_H
#define FEM_BVH_BUILDER_H
    ...
    sizeType buildBVH(vector<NODE>& bvh) {
        ...
        for(sizeType d=0; d<DIM; d++) {
            for(sizeType i=0; i<(sizeType)_roots.size(); i++) {
                _hdls[d].push_back(BVHHandle(bvh[_roots[i]]._bb._minC[d],_roots[i],true));
                _hdls[d].push_back(BVHHandle(bvh[_roots[i]]._bb._maxC[d],_roots[i],false));
            }
...
struct Node : public Serializable {
    ...
    boost::shared_ptr<Serializable> copy() const {
        return boost::shared_ptr<Serializable>(new Node<T>);
    }
    BBox<scalar> _bb;
    T _cell;
...
template <typename T>
void buildBVH(vector<Node<T> >& bvh,sizeType dim,T verbose)
{
    sizeType nrBVH=(sizeType)bvh.size();
    if(dim == 2)BVHBuilder<Node<T>,2>().buildBVH(bvh);
    else BVHBuilder<Node<T>,3>().buildBVH(bvh);
    for(sizeType i=nrBVH; i<(sizeType)bvh.size(); i++)
        bvh[i]._cell=verbose;
}

template <typename T>
static void writeBVHByLevel(const vector<Node<T> >& bvh,T verbose)
{
    if(bvh.empty())
        return;
    deque<sizeType> lv;
    ...
    for(sizeType i=0; !lv.empty(); i++) {
        ...
        for(sizeType i=0; i<nrN; i++) {
            const Node<T>& n=bvh[lv.front()];
            BBox<scalar> bb=n._bb;
...
public:
    ...
    struct AddCache {
        ...
        void onCell(const Node<T>& n,const Node<T2>& n2) {
            _cache.push_back(CACHE(n._cell,n2._cell));
        }
        std::vector<CACHE>& _cache;
    };
    BVHQuery(const vector<Node<T> >& bvh,sizeType dim,T verbose)
        :_bvh(bvh),_dim(dim),_verbose(verbose) {}
    void updateBVH(scalar expand=1E-3f) {
        sizeType nrN=(sizeType)_bvh.size();
        for(sizeType i=0; i<nrN; i++) {
            Node<T>& n=const_cast<Node<T>&>(_bvh[i]);
            if(n._cell != _verbose)
    ...
    void pointQuery(const Vec3& pos,scalar eps,CALLBACK& cb,scalar& sqrDist) const {
        if(_bvh.empty())
            return;
        BBox<scalar> bb(pos-Vec3::Constant(eps),pos+Vec3::Constant(eps));
        std::stack<sizeType> ss;
        ss.push((sizeType)_bvh.size()-1);
        while(!ss.empty()) {
            const Node<T>& node=_bvh[ss.top()];
            ss.pop();
            if(!node._bb.intersect(bb,_dim))
                continue;
    ...
    void interBodyQuery(const BVHQuery<T2>& query2,CALLBACK& cb) const {
    ...
    template <typename T2,typename CALLBACK>
    void interBodyDepthQuery(const BVHQuery<T2>& query2,CALLBACK& cb,scalar depth) const {
        if(_bvh.empty() || query2._bvh.empty())
            return;
        std::stack<Vec2i> ss;
        ss.push(Vec2i(_bvh.size()-1,query2._bvh.size()-1));
    ...
    template <typename T2,typename CACHE>
    void broadphaseQuery(const BVHQuery<T2>& query2,std::vector<CACHE>& cache) const {
        AddCache<T2,CACHE> cb(cache);
        interBodyQuery(query2,cb);
    }
    const vector<Node<T> >& _bvh;
    sizeType _dim;
    T _verbose;
};
