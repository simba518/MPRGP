...
struct BoxGeomCell : public FEMGeomCell {
    BoxGeomCell():FEMGeomCell(0) {}
    BoxGeomCell(const Mat4& T,sizeType dim,const Vec3& ext):FEMGeomCell(T,dim,0),_ext(ext) {}
    virtual void getMesh(ObjMeshTpl<scalar>& mesh) const {
        if(_dim == 2)MakeMesh::makeBox2D(mesh,_ext);
        else MakeMesh::makeBox3D(mesh,_ext);
        FEMGeomCell::getMesh(mesh);
    }
...
struct SphereGeomCell : public FEMGeomCell {
    SphereGeomCell():FEMGeomCell(1) {}
    SphereGeomCell(const Mat4& T,sizeType dim,scalar rad):FEMGeomCell(T,dim,1),_rad(rad) {}
    virtual void getMesh(ObjMeshTpl<scalar>& mesh) const {
...
struct CylinderGeomCell : public FEMGeomCell {
    CylinderGeomCell():FEMGeomCell(2) {}
    CylinderGeomCell(const Mat4& T,sizeType dim,scalar rad,scalar y):FEMGeomCell(T,dim,2),_rad(rad),_y(y) {}
    virtual void getMesh(ObjMeshTpl<scalar>& mesh) const {
...
struct ObjMeshGeomCell : public FEMGeomCell {
    ObjMeshGeomCell():FEMGeomCell(3),_query(_bvh,_dim,-1),_insideOut(false) {}
    ObjMeshGeomCell(const Mat4& trans,const ObjMeshTpl<scalar>& mesh,scalar depth,bool insideOut)
        :FEMGeomCell(trans,mesh.getDim(),3),_depth(depth),_query(_bvh,_dim,-1),_insideOut(insideOut) {
        _vss=mesh.getV();
        _iss=mesh.getI();
...
//Geom
FEMGeomCell::FEMGeomCell(sizeType type):Serializable(type) {}
FEMGeomCell::FEMGeomCell(const Mat4& T,sizeType dim,sizeType type):Serializable(type),_T(T),_invT(T.inverse()),_dim(dim) {}
void FEMGeomCell::getMesh(ObjMeshTpl<scalar>& mesh) const
{
...
BBox<scalar> FEMGeomCell::getBB() const
{
    Vec3 pt;
...
bool FEMGeomCell::dist(const Vec3& pt,Vec3& n) const
{
...
bool FEMGeomCell::read(std::istream& is)
{
...
bool FEMGeomCell::write(std::ostream& os) const
{
...
//Static Geometry
FEMGeom::FEMGeom(sizeType dim):HasMagic(0xaabbccdd),_dim(dim)
{
    _bvh.reset(new vector<Node<boost::shared_ptr<FEMGeomCell> > >);
}
const vector<Node<boost::shared_ptr<FEMGeomCell> > >& FEMGeom::getBVH() const
{
    return *_bvh;
}
void FEMGeom::assemble()
{
    _bvh->clear();
    for(sizeType i=0; i<(sizeType)_css.size(); i++) {
        _css[i]->_index=i;
        Node<boost::shared_ptr<FEMGeomCell> > n;
    ...
    buildBVH(*_bvh,_dim,boost::shared_ptr<FEMGeomCell>());
}
void FEMGeom::addGeomCell(boost::shared_ptr<FEMGeomCell> c)
{
    _css.push_back(c);
}
void FEMGeom::addGeomBox(const Mat4& trans,const BBox<scalar>& bb)
{
    Mat4 T=Mat4::Identity();
    T.block<3,1>(0,3)=(bb._maxC+bb._minC)/2.0f;
    addGeomBox(trans*T,bb.getExtent()*0.5f);
}
void FEMGeom::addGeomBox(const Mat4& trans,const Vec3& ext)
{
    _css.push_back(boost::shared_ptr<FEMGeomCell>(new BoxGeomCell(trans,_dim,ext)));
}
void FEMGeom::addGeomBox(const OBBTpl<scalar,2>& obb)
{
    ASSERT(_dim == 2)
...
void FEMGeom::addGeomBox(const OBBTpl<scalar,3>& obb)
{
    ASSERT(_dim == 3)
    Mat4 m=Mat4::Identity();
...
void FEMGeom::addGeomPlane(const Mat4& trans,const Vec4& plane,scalar ext)
{
    ...
    if(_dim == 3)
        _css.push_back(boost::shared_ptr<FEMGeomCell>(new CylinderGeomCell(trans*T,_dim,ext,ext)));
    else _css.push_back(boost::shared_ptr<FEMGeomCell>(new BoxGeomCell(trans*T,_dim,Vec3::Constant(ext))));
}
void FEMGeom::addGeomPlane(const Mat4& trans,const PlaneTpl<scalar>& plane,scalar ext)
{
    Vec4 p;
...
void FEMGeom::addGeomSphere(const Vec3& ctr,scalar rad)
{
    Mat4 T=Mat4::Identity();
    T.block<3,1>(0,3)=ctr;
    _css.push_back(boost::shared_ptr<FEMGeomCell>(new SphereGeomCell(T,_dim,rad)));
}
void FEMGeom::addGeomMesh(const Mat4& trans,const ObjMeshTpl<scalar>& mesh,scalar depth,bool insideOut)
{
    _css.push_back(boost::shared_ptr<FEMGeomCell>(new ObjMeshGeomCell(trans,mesh,depth,insideOut)));
}
void FEMGeom::addGeomMesh(const Mat4& trans,const std::string& path,scalar depth,bool insideOut)
{
...
//IO
void FEMGeom::writeVTK(const std::string& path) const
{
    ObjMeshTpl<scalar> mesh;
...
void FEMGeom::writeBVH() const
{
    writeBVHByLevel<boost::shared_ptr<FEMGeomCell> >(*_bvh,boost::shared_ptr<FEMGeomCell>());
}
bool FEMGeom::write(std::ostream& os) const
{
...
bool FEMGeom::read(std::istream& is)
{
    _css.clear();
    _bvh->clear();
